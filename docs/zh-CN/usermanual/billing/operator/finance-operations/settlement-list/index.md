...
| 典型用途 | 查询结算单、查看结算状态、进入结算详情、生成结算单 |
...
1. 点击 `生成结算单`。
2. 选择目标账期和组织。
3. 查看生成检查结果，确认没有阻塞性异常。
4. 核对结算预览中的组织、账期、应付组织金额和结算状态。
5. 填写备注时，只写脱敏后的处理说明，不写真实银行账号、合同编号、客户敏感信息或内部处理意见。
6. 提交生成后，回到列表按账期和组织搜索。
7. 进入详情确认状态、金额和入账确认信息。
...
- 不要在账期数据未核对完成前生成结算单。
- 不要重复生成同一组织、同一账期的结算单。
- 备注中不要写入真实银行账号、合同编号、客户敏感信息或内部处理意见。
- 生成失败时不要反复点击，先进入巡检中心或详情页确认失败原因。
