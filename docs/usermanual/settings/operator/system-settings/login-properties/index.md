...
| Navigation path | System Settings > Login Properties |
| Page route | /operator/system-settings/login-properties |
...
3. Select the target row or entry related to login properties records and related status.
4. Click the visible `View Login Properties` entry when it is available.
...
The expected result is not visible on the `Login Properties` page, or the available action does not match the current business expectation.
...
- The action may be restricted because it affects login properties records and related status.

**Handling:**

1. Reset filters and search again from `System Settings > Login Properties`.
...
### What to Check on Login Properties
