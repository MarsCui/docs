...
`集群管理` 用于将 Kubernetes 集群（用于管理计算节点、容器和作业调度的容器编排系统）接入 AI Infra-On Prem 资源池，并持续管理集群规格、共享存储、节点状态、作业分布和资源监控。运营方完成集群注册后，平台才能在对应地域和可用区中调度开发、训练、推理等作业。
...
所以注册集群的核心目的，是把实际算力接入平台调度系统。没有完成集群注册，即使地域和可用区已经创建，平台也没有可调度的节点资源。

### 资源层级关系

```mermaid
flowchart LR
  A["地域<br/>资源大区"] --> B["可用区<br/>资源分组"]
  B --> C["集群<br/>服务器群组"]
  C --> D["节点<br/>单台机器"]
  C --> E["规格<br/>资源套餐"]
  C --> F["存储<br/>共享目录"]
  D --> G["作业<br/>训练、推理、在线 IDE 等"]
  E --> G
  F --> G
```
...
> ⚠️ 安全提示
>
> 不要在文档、截图、工单、提交记录或评论中写入证书、私钥、token、密码、AK/SK 或完整 kubeconfig。
...
- **认证材料**：可能是客户端证书/私钥、用户名密码、token 或 auth-provider 信息。不同认证方式需要填写的字段不同。
...
| --- | --- | --- | --- | --- |
| 地域 | 是 | 下拉选择 | `武汉` | 集群所属地域。需先在 `资源池 > 地域/可用区` 中创建。 |
| 可用区 | 是 | 下拉选择 | `武汉-1` | 集群所属可用区，依赖已选地域。 |
| 注册名称 | 是 | 文本 | `prod-wuhan-gpu-1` | 平台内集群识别名称，需避免重复。推荐使用小写字母、数字和连字符组合，体现环境、地域和用途，例如 `prod-wuhan-gpu-1`。创建前请确认命名。 |

#### 集群连接信息

| 字段名称 | 是否必填 | 字段类型 | 示例 | 填写说明 |
| --- | --- | --- | --- | --- |
| 身份验证证书 | 是 | 多行文本 | base64 证书内容 | Kubernetes 集群 CA 证书，用于校验 API Server 身份。不要在文档或截图中展示真实证书内容。 |
| 服务器地址 | 是 | URL | `https://cluster-api.example.local:6443` | Kubernetes API Server 访问地址。填写前应确认平台侧网络可达。 |
| 集群名称 | 是 | 文本 | `cluster1` | kubeconfig 中的 Kubernetes cluster 名称，用于与 context 关联。 |

#### 用户认证类型

| 认证方式 | 页面取值 | 需要填写的信息 | 说明 |
| --- | --- | --- | --- |
| 证书认证 | `client` | `client-certificate`、`client-key` | 适合使用客户端证书访问 API Server 的集群。私钥属于敏感信息，禁止外泄。 |
| 用户名密码认证 | `password` | 用户名、密码 | 适合仍保留用户名密码认证方式的环境。 |
| 身份令牌认证 | `token` | token | 适合使用 Bearer Token 访问 API Server 的环境。 |
| 认证程序认证 | `auth-provider` | auth-provider 相关字段 | 适合由外部认证程序提供凭证的环境，需按集群管理员提供的信息填写。 |

#### context

| 字段名称 | 是否必填 | 字段类型 | 示例 | 填写说明 |
| --- | --- | --- | --- | --- |
| context 名称 | 自动生成 | 文本 | `cluster1-admin` | context 是 kubeconfig 中的连接配置组合，用来关联集群、用户和命名信息。 |
| 集群名称 | 自动带入 | 文本 | `cluster1` | 与上方集群连接信息保持一致。 |
| 用户名称 | 自动带入 | 文本 | `admin` | 与认证信息中的用户保持一致。 |

#### 高级配置

| 字段名称 | 是否必填 | 字段类型 | 示例 | 填写说明 |
| --- | --- | --- | --- | --- |
| 支持 RDMA 网络 | 否 | 开关 | 开启 / 关闭 | RDMA 高速网络能力相关高级选项。仅在硬件、驱动和网络规划明确支持时开启；不确定时保持关闭并联系平台运维。 |
| 管理网开端口 | 是 | 数字范围 | `30000` - `32760` | 管理网开放端口范围，通常用于 NodePort。当前页面输入框支持 `1-65535`，实际开放范围应与网络规划一致。 |
| 外网 IP/域名 | 是 | URL / 域名 | `https://cluster.example.local` | 集群对外暴露服务或平台侧访问入口。API Server 地址用于平台连接 Kubernetes 控制面；外网 IP/域名用于访问集群对外暴露的服务或平台侧访问入口，二者用途不同。 |
| cluster CIDR | 是 | CIDR | `172.20.0.0/16` | Pod IP 网段。需避免与平台、节点、服务网段冲突。 |
| service CIDR | 是 | CIDR | `10.68.0.0/16` | Service IP 网段。需避免与现有网络冲突。 |
| POD 内部禁止访问网段 | 否 | 动态列表 | `10.0.0.0/8` | 限制 Pod 内部访问的网段，按安全策略配置。 |
| 监控服务端口 | 是 | 数字 | `32761` | 监控服务端口。需确认端口未被占用且可访问。 |
| jupyterlab 地址 | 是 | URL | `https://jupyter.example.local` | JupyterLab 访问地址。用于平台关联在线开发入口。 |
| 描述 | 否 | 多行文本 | `生产环境 GPU 集群` | 用于记录集群用途、环境、负责人或维护说明。 |
...
| 字段名称 | 是否必填 | 字段类型 | 示例 | 填写说明 |
| --- | --- | --- | --- | --- |
| 名称 | 是 | 文本 | `prod-model-nfs` | 存储卷标识名称，建议能体现用途、环境和范围。 |
| 类型 | 是 | 单选 | `nfs` / `hostpath` | `nfs` 适合网络共享目录；`hostpath` 适合挂载宿主机本地目录。 |
| 政策 | 是 | 单选 | `读写` / `只读` | 存储访问权限。生产环境应按最小权限原则配置。 |
| 文件存储服务 | 条件必填 | 文本 | `nfs.example.local` | 选择 `nfs` 时填写 NFS 服务地址；选择 `hostpath` 时通常不需要。 |
| 共享路径 | 是 | 文本 | `/data/models` | 宿主机或 NFS 服务端的共享目录。需确认路径存在且权限正确。 |
| 容器内路径 | 是 | 文本 | `/mnt/models` | 作业容器内挂载路径。避免与系统目录或应用目录冲突。 |
| 租户范围 | 是 | 单选 | `所有租户共享` / `租户有独立子目录` / `el表达式` | 控制不同租户访问同一存储时的隔离方式。`el表达式` 是高级选项，用于按规则动态生成租户目录，一般场景不需要使用。 |
| 本地git仓库 | 是 | 开关 | 开启 / 关闭 | 是否作为本地 Git 仓库目录。 |
| 本地模型仓库 | 是 | 开关 | 开启 / 关闭 | 是否作为本地模型仓库目录。 |
...
编辑前应确认是否有运行中的作业正在使用该存储。路径或权限变更可能导致作业读写失败。
...
- **集群接入依赖**：API Server 可达、认证材料有效、CIDR 和端口规划正确，是集群可接入的基础条件。
...
**现象**：集群已出现在列表中，但状态不是可用，或资源信息无法正常加载。
...
**现象**：用户创建作业时无法选择某个规格，或规格列表中没有目标规格。
...
**现象**：集群已经注册，但注册名称含义不清，后续排查地域、环境、用途或容量归属时难以判断。
...
**现象**：添加存储时不确定应该选择 `nfs` 还是 `hostpath`。
...
> ⚠️ 安全提示
>
> 不要在文档、截图、工单、提交记录或评论中写入真实账号、密码、token、AK/SK、私钥、证书或完整 kubeconfig。
