...
`客户调用-调用日志` 用于查看客户单次请求记录，包括请求 ID、错误码、客户标识和返回摘要，是客户问题排查入口。
...
| 检查项 | 成功表现 | 异常时处理 |
| --- | --- | --- |
| 能按客户、请求 ID、模型、状态 | 能按客户、请求 ID、模型、状态和时间范围筛选日志。 | 未达到时回到对应页面核对权限、筛选条件和配置状态 |
| 日志详情展示错误码、延迟、Tok | 日志详情展示错误码、延迟、Token 和脱敏响应摘要。 | 未达到时回到对应页面核对权限、筛选条件和配置状态 |
| 客户反馈的问题能定位到对应请求或 | 客户反馈的问题能定位到对应请求或时间段。 | 未达到时回到对应页面核对权限、筛选条件和配置状态 |
## 常见问题
...
客户提供了大致时间或错误描述，但调用日志中找不到对应请求。

**可能原因：**

请求发生在其他模型、客户标识或时间范围下，客户提供的请求 ID 不完整，或日志存在采集延迟。

**处理方式：**

扩大时间范围并按客户、模型和状态组合筛选；让客户提供脱敏请求 ID 和时间；仍找不到时检查日志采集状态。
