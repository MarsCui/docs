...
::: warning 安全提示
配额、额度和计量截图中不要暴露真实企业编号、租户名称、账务金额、消费明细或业务标识。对账材料应按组织要求脱敏。
:::
...
![计量明细页面](./images/metering-details-list-browser.png)
...
| --- | --- | --- | --- | --- |
| 租户 | 是 | 下拉选择 | `tenant-a` | 过滤需要查看的消费主体。 |
| 资源名称 | 条件必填 | 文本 | `inference-qwen` | 产生计量的实例、作业或存储资源。 |
| 计费周期 | 是 | 日期范围 | `2026-07-01 至 2026-07-31` | 明细统计所属的结算周期。 |
...
- 配额充足不代表底层集群一定有空闲资源。
- 计量数据可能延迟，核对时统一时间范围和统计口径。
- 导出数据前脱敏租户、金额和业务标识。

### 结果校验

1. 筛选结果与条件一致。
2. 明细起止时间和资源类型可解释月度汇总。
