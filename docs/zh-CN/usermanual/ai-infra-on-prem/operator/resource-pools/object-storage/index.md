...
1. 进入 `AI Infra > On-Prem > 资源池管理 > 对象存储组件`。
2. 点击 `注册组件`、`新增` 或页面真实注册入口。
3. 按页面字段填写组件名称、Endpoint、内网地址、访问协议、认证方式、容量控制和连接信息。
4. 根据页面要求配置访问凭据、Bucket 能力、容量限制、关联地域或可见范围。
5. 点击最终 `保存`、`提交` 或 `确定` 前，再次核对 Endpoint、内网地址、AK/SK 来源、Bucket 策略和地域绑定范围。
...
| 参数 | 是否必填 | 说明 | 配置建议 |
| --- | --- | --- | --- |
| 组件名称 | 必填 | 对象存储组件展示名称。 | 建议体现存储类型、环境或地域，不使用临时命名。 |
| Endpoint | 必填 | 对象存储对平台或业务侧暴露的访问入口。 | 确认平台侧和集群侧都能访问；文档中不写真实 Endpoint。 |
| 内网地址 | 条件必填 | 集群或平台访问对象存储的内部网络地址。 | 与实际网络、DNS 和路由配置一致；不要写真实内网地址。 |
| 访问协议 | 必填 | 对象存储兼容协议或访问方式。 | 按页面支持能力选择 S3 兼容协议或实际协议。 |
| 认证方式 | 必填 | 对象存储访问认证方式。 | 按平台支持选择 AK/SK、Token 或其他认证方式。 |
| 访问凭据 | 条件必填 | AK/SK、Token 或连接凭据。 | 只在系统表单中填写，不写入文档、截图或工单。 |
| Bucket 能力 | 条件必填 | 是否支持创建、同步、读取或写入 Bucket。 | 与用户侧对象存储能力和租户隔离策略保持一致。 |
| 容量限制 | 否 | 容量配额、桶数量或对象数量限制。 | 根据租户、地域和作业规模规划。 |
| 关联地域 | 条件必填 | 对象存储组件可绑定或可见的地域范围。 | 与资源池、可用区、集群和业务数据范围一致。 |
| 状态 | 系统生成 | 组件注册、连通性和探测状态。 | 注册后关注状态、更新时间和错误提示。 |
| 操作 | 否 | 支持注册、编辑、启用、禁用、删除或测试连接等操作。 | 高风险操作前确认地域、数据和作业影响。 |
...
| 组件列表正常加载 | 组件名称、Endpoint、内网地址、容量、状态和关联地域正常显示。 | 刷新页面并检查服务状态或浏览器控制台错误。 |
| 注册入口可见 | 页面显示 `注册组件`、`新增` 或真实注册入口。 | 检查运营方权限、License 和页面配置。 |
| 注册表单可打开 | 点击入口后可查看组件名称、Endpoint、内网地址、认证方式和容量控制字段。 | 检查路由、权限和前端错误。 |
| 必填字段校验正常 | 未填写组件名称、Endpoint、认证信息或容量字段时出现校验提示。 | 按页面提示补齐字段，不绕过校验。 |
