...
| --- | --- | --- | --- | --- |
| 应用名称 | 必填 | 文本 | `customer-assistant` | 应用展示名称。 |
| 绑定模型 | 必填 | 下拉选择 | `qwen-plus` | 应用调用的模型。 |
| 可见范围 | 必填 | 多选 | `指定客户` | 控制哪些客户可见。 |
...
| 检查项 | 成功表现 | 异常时处理 |
| --- | --- | --- |
| 应用发布状态与操作结果一致 | 应用发布状态与操作结果一致。 | 未达到时回到对应页面核对权限、筛选条件和配置状态 |
| 客户视角能按可见范围看到应用 | 客户视角能按可见范围看到应用。 | 未达到时回到对应页面核对权限、筛选条件和配置状态 |
| 应用调用入口与绑定模型配置一致 | 应用调用入口与绑定模型配置一致。 | 未达到时回到对应页面核对权限、筛选条件和配置状态 |
## 常见问题
