...
`我的调用-调用日志` 用于查看当前调用方的单次请求详情，包括请求 ID、错误码、延迟、Token 和上游返回摘要。
...
| 检查项 | 成功表现 | 异常时处理 |
| --- | --- | --- |
| 能按请求 ID、状态、模型或时间 | 能按请求 ID、状态、模型或时间范围定位记录。 | 未达到时回到对应页面核对权限、筛选条件和配置状态 |
| 日志详情展示错误码、延迟、Tok | 日志详情展示错误码、延迟、Token 和脱敏摘要。 | 未达到时回到对应页面核对权限、筛选条件和配置状态 |
| 失败请求能关联到可执行的处理建议 | 失败请求能关联到可执行的处理建议。 | 未达到时回到对应页面核对权限、筛选条件和配置状态 |
## 常见问题
...
调用日志中有请求记录，但详情里的返回摘要为空或不完整。

**可能原因：**

请求失败未生成模型输出，返回内容被安全策略截断，或摘要字段因脱敏规则被隐藏。

**处理方式：**

先查看状态码、错误码和耗时；确认是否命中安全或脱敏规则；需要进一步排查时提供请求 ID 给运营方。
