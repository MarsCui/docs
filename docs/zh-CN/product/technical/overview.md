...
版本：v1.1
更新日期：2026-07-13
功能事实基线：2026-07-08 更新的用户手册
::::

## 产品定位

AGIOne 是面向企业级场景的 AI 智算平台，围绕算力统一纳管、多源模型管理与企业级治理，连接算力准备、模型部署与发布、模型体验与调用以及用量运营等任务。

平台由 AI Infra On-Prem、AI Infra On-Cloud 和 Model Services 三个子系统组成：On-Prem 管理本地集群与算力资源，On-Cloud 管理受支持云平台的账号、授权和部署资源，Model Services 管理模型发布、审核、体验、调用与收益。具体能力取决于部署版本、账号角色、授权范围和已准备资源。
...
平台通过租户、角色和资源授权范围区分管理员、运营方、模型提供方与普通用户的职责，并提供配额、额度、审核和调用记录等治理入口。实际可见菜单和数据范围取决于账号权限与当前环境配置。

### 4. 全链路可观测

平台提供集群、节点、设备、作业、部署状态、调用日志、调用分析、用量和计量等页面，为结果校验和排障提供依据。具体指标、统计范围和同步时效因子系统、角色与部署版本而异。
...
- **AI 应用监控与运维治理**：结合监控、调用日志、调用分析、配额和计量页面完成运行检查
...
- **统一模型管理与运维入口**：集中查看模型配置、审核、部署状态、调用记录、用量和收益等对象
- **在权限边界内管理资源使用**：通过租户、角色、授权、配额与计量控制不同账号的可见范围和使用边界
