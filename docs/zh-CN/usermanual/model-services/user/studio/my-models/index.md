...
`我的模型` 用于管理自有模型、模型来源、请求头、Endpoint、发布方式、计费和限流配置，是提供方发布模型的工作台。
...
| --- | --- | --- | --- | --- |
| 发布方式 | 必填 | 枚举 | `BYOK` | 单模型、BYOK 或聚合模型。 |
| 元模型 | 必填 | 下拉选择 | `Qwen Text` | 定义能力和协议。 |
...
| 检查项 | 成功表现 | 异常时处理 |
| --- | --- | --- |
| 模型草稿、审核状态或发布状态与操 | 模型草稿、审核状态或发布状态与操作结果一致。 | 未达到时回到对应页面核对权限、筛选条件和配置状态 |
| BYOK 模型能通过连通性或协议 | BYOK 模型能通过连通性或协议测试。 | 未达到时回到对应页面核对权限、筛选条件和配置状态 |
| 聚合模型的候选模型和路由策略保存 | 聚合模型的候选模型和路由策略保存正确。 | 未达到时回到对应页面核对权限、筛选条件和配置状态 |
## 常见问题
