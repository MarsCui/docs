...
| Navigation path | System Settings > Platform Settings |
| Page route | /operator/system-settings/platform-settings |
...
3. Select the target row or entry related to platform settings records and related status.
4. Click the visible `View Platform Settings` entry when it is available.
...
### Platform Settings Troubleshooting

**Issue Symptom:**

The expected result is not visible on the `Platform Settings` page, or the available action does not match the current business expectation.
...
- The action may be restricted because it affects platform settings records and related status.

**Handling:**

1. Reset filters and search again from `System Settings > Platform Settings`.
