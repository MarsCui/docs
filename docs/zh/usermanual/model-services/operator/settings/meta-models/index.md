...
| 适用角色 | 运营方 |
| 导航路径 | 设置 > 元模型 |
| 功能定位 | 维护平台支持的元模型（Meta-model）数据，包括模型作者和具体模型版本，是模型发布、模板创建等流程的基础数据源 |

## 页面结构

### 搜索区域

页面顶部支持按模型作者名称快速定位目标元模型。

### 操作按钮区

- 页面顶部提供 **"添加"** 按钮，用于新增模型作者。
- 顶部提供 **"导出"** / **"导入"** 按钮，批量管理模型作者或元模型配置。
- 每个模型作者卡片提供 **"编辑"** / **"删除"** 操作。
- 每个元模型行提供 **"+ 添加"**、**"编辑"**、**"复制"**、**"详情"**、**"启用"** / **"禁用"**、**"删除"** 等操作。

### 数据列表说明

- **模型作者列表**：以卡片形式展示所有作者，每个作者显示唯一标识、显示名称（多语言）、应用图标。
- **元模型列表**：在选中作者后展示该作者下的所有元模型，显示唯一标识、名称、系列、状态、版本、Token 限制、模型类型等关键信息。
...
#### **Step 2：元模型详情**：
- **元模型详情**：在富文本编辑器中填写模型的完整详细介绍（支持富文本格式、插入链接等）。

![元模型详情](./images/step-2-meta-model-details.png)

- 确认所有信息无误后，点击 **"提交"** 按钮完成添加。
...
| 操作名称 | 操作步骤 |
|----------|----------|
| 编辑模型作者 | 在左侧模型作者列表，点击目标作者的 **"编辑"** 按钮 → 修改唯一标识、显示名称、应用图标等信息 → 点击 **"确定"** |
| 编辑元模型 | 在元模型列表，点击目标元模型的 **"编辑"** 按钮 → 修改元模型配置 → 点击 **"提交"** |
| 复制元模型 | 点击目标元模型的 **"复制"** 按钮 → 基于现有模型快速创建新的元模型 |
| 查看元模型详情 | 点击目标元模型的 **"详情"** 按钮 → 查看完整的模型配置与介绍信息 |
| 启用 / 禁用元模型 | 点击目标元模型的 **"启用"** / **"禁用"** 按钮 → 切换模型的可用状态 |
| 删除模型作者 | 在左侧模型作者列表，点击目标作者的 **"删除"** 按钮 → 确认操作（**删除操作不可逆，请谨慎操作**） |
| 删除元模型 | 点击目标元模型的 **"删除"** 按钮 → 确认操作（**删除操作不可逆，请谨慎操作**） |
| 导出 / 导入配置 | 点击页面右上角的 **"导出"** / **"导入"** 按钮 → 批量管理模型作者或元模型配置 |

## 注意事项

- **删除操作不可逆**，请谨慎操作。
- 模型作者删除前需先清空该作者下的所有元模型。
- 元模型是模板、发布、模型市场等流程的基础数据，修改后可能影响已发布的模型配置。
