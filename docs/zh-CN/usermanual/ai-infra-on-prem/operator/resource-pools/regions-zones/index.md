...
| --- | --- | --- | --- | --- |
| 地域名称 | 必填 | 文本 / 多语言 | `武汉` | 平台展示的地域名称，建议与实际机房或资源池区域一致。 |
| 可用区 | 条件必填 | 文本 | `wuhan-1` | 地域下的资源隔离单元，后续集群需绑定到可用区。 |
| 状态 | 系统生成 | 枚举 | `正常` | 地域当前可用状态，影响后续资源创建和调度。 |
| 关联集群 | 系统生成 | 列表 | `cluster-prod-a` | 当前地域或可用区下已经接入的集群。 |
| 绑定组件 | 条件必填 | 多选 | `harbor-prod / minio-prod` | 地域可使用的镜像、对象存储、文件存储或块存储组件。 |
...
| 可用区 ID | 必填 | 文本 | `wuhan-1` | 可用区唯一标识。推荐使用小写字母、数字和连字符组合，如 `wuhan-1`、`wuhan-gpu-1`。创建后不可修改。 |
...
筛选项说明见前文 `页面说明 > 筛选区`。筛选地域时按以下步骤操作：

1. 在顶部筛选区选择地域类型、状态或输入地域名称。
2. 点击 `搜索` 查看筛选结果。
3. 如需切换筛选条件，可调整筛选项后再次点击 `搜索`。
4. 如需恢复默认列表，点击 `重置`。
...
当某个地域需要维护、下线或临时停止承载新资源时，可禁用地域。禁用前应确认该地域下有没有可用区、集群或运行中的作业。
