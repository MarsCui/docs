...
| --- | --- | --- | --- | --- |
| 框架名称 | 是 | 文本 | `VLLM` | 底层使用的推理框架或引擎名称，建议采用官方框架命名，便于识别和技术对接。 |
| 版本名称 | 是 | 文本 | `v1.3.0` | 框架配置的版本标识，用于跟踪迭代或兼容性管理。 |
| 镜像 | 是 | 镜像地址 | `registry.example.com/runtime/vllm:1.0` | 运行推理任务所需的容器镜像。镜像与地域存在关联，需确保目标地域可用。 |
| 主节点启动命令 | 是 | 命令行 | `python -m vllm.entrypoints.openai.api_server` | 任务集群中主节点的启动命令；单节点任务时直接作为该节点启动命令。 |
...
| 服务端口 | 是 | 数字 | `8000` | 平台探测、路由或暴露服务时使用的监听端口。 |
