...
`我的接入账号` 用于维护用户侧云账号、访问密钥、授权状态和可用资源范围，用于普通用户管理可用于部署的云账号和密钥。
...
| --- | --- | --- | --- | --- |
| 账号名称 | 必填 | 文本 | `my-cloud-account` | 用户侧展示名称。 |
| 云平台 | 必填 | 下拉选择 | `阿里云` | 账号所属云平台。 |
...
| 检查项 | 成功表现 | 异常时处理 |
| --- | --- | --- |
| 账号列表显示授权状态和可用地域 | 账号列表显示授权状态和可用地域。 | 未达到时回到对应页面核对权限、筛选条件和配置状态 |
| 快速部署页能看到与账号匹配的云资 | 快速部署页能看到与账号匹配的云资源。 | 未达到时回到对应页面核对权限、筛选条件和配置状态 |
| 无权限或未授权账号不会出现在可选 | 无权限或未授权账号不会出现在可选资源中。 | 未达到时回到对应页面核对权限、筛选条件和配置状态 |
...
- 用户侧账号授权以当前租户和业务地域为边界，不等同于云厂商账号的全部权限。
- 云账号权限变化可能影响已有部署的扩缩容、重建和监控同步，应先评估影响范围。
- 需要运营方介入时只提供脱敏后的账号标识、地域、资源池和失败时间。