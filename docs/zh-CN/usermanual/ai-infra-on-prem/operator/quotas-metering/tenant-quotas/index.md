...
| 检查项 | 成功表现 | 异常时处理 |
| --- | --- | --- |
| 默认额度策略保存成功 | 默认额度策略保存成功。 | 未达到时检查租户、账期、额度、用量记录和计量同步状态 |
...
| 检查项 | 成功表现 | 异常时处理 |
| --- | --- | --- |
| 客户出现在列表中 | 客户出现在列表中。 | 未达到时检查租户、账期、额度、用量记录和计量同步状态 |
| 充值明细中有对应初始化记录 | 充值明细中有对应初始化记录。 | 未达到时检查租户、账期、额度、用量记录和计量同步状态 |
...
| --- | --- | --- | --- | --- |
| 租户 | 必填 | 下拉选择 | `tenant-a` | 需要配置或查看配额的租户。 |
| 资源类型 | 必填 | 枚举 | `GPU` | 配额约束的资源类别，如 GPU、CPU、内存、存储或实例数。 |
| 配额上限 | 必填 | 数字 / 容量 | `8 卡` | 租户可使用的最大资源额度。 |
...
| 检查项 | 成功表现 | 异常时处理 |
| --- | --- | --- |
| 目标客户额度变化符合预期 | 目标客户额度变化符合预期。 | 未达到时检查租户、账期、额度、用量记录和计量同步状态 |
| 明细记录可追溯 | 明细记录可追溯。 | 未达到时检查租户、账期、额度、用量记录和计量同步状态 |
