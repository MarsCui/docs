...
| 参数 | 是否必填 | 说明 | 配置建议 |
| --- | --- | --- | --- |
| 名称 | 必填 | 文件存储组件名称。 | 建议体现环境、用途或容量边界。 |
| 限制租户使用量 | 必填 | 是否限制租户可用容量。 | 与租户容量策略一致。 |
| 超分比例 | 必填 | 文件存储逻辑容量和实际容量的超分比例。 | 按容量规划谨慎填写。 |
| 实际容量阈值 | 必填 | 实际容量告警或限制阈值。 | 不要超过真实容量安全边界。 |
| 超分容量阈值 | 必填 | 超分后逻辑容量告警或限制阈值。 | 与超分比例联动核对。 |
| 粘贴配置 | 条件必填 | 通过粘贴 kubeconfig 内容录入集群连接信息。 | 不在文档中写真实 kubeconfig。 |
| 上传文件 | 条件必填 | 通过上传文件录入集群连接信息。 | 仅上传受控环境中的授权配置文件。 |
| 手动录入 | 条件必填 | 通过手动字段录入集群连接信息。 | 不写真实内网地址、账号或密钥。 |
| 描述 | 否 | 组件用途、边界或维护说明。 | 只记录非敏感说明。 |
| 操作 | 系统生成 | 测试连接、提交、搜索、重置等入口。 | `提交` 会提交真实配置；学习或截图时不要误触。 |
