...
| 模型名称 | 必填 | 文本 | `qwen-plus` | 申请上架或更新的模型。 |
| 供应方 | 必填 | 文本 | `provider-a` | 模型来源或提供方。 |
| 审核结论 | 必填 | 枚举 | `通过` | 通过、驳回或补充材料。 |
...
| 检查项 | 成功表现 | 异常时处理 |
| --- | --- | --- |
| 审核结论保存后 | 审核结论保存后，审核单状态同步更新。 | 未达到时回到对应页面核对权限、筛选条件和配置状态 |
| 通过后模型进入发布或上架流程 | 通过后模型进入发布或上架流程。 | 未达到时回到对应页面核对权限、筛选条件和配置状态 |
| 驳回或补充材料时 | 驳回或补充材料时，申请方能看到明确原因。 | 未达到时回到对应页面核对权限、筛选条件和配置状态 |
## 常见问题
