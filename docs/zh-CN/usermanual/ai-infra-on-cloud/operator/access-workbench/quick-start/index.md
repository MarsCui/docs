...
`快速接入` 用于维护云平台、云账号、资源池、租户授权和部署资产，用于按接入向导完成云平台、账号、资源池和授权配置。
...
| 检查项 | 成功表现 | 异常时处理 |
| --- | --- | --- |
| 向导关键步骤均显示完成 | 向导关键步骤均显示完成。 | 未达到时回到对应页面核对权限、筛选条件和配置状态 |
| 资源池和授权页能看到对应配置 | 资源池和授权页能看到对应配置。 | 未达到时回到对应页面核对权限、筛选条件和配置状态 |
| 测试部署能创建并进入运行或可诊断 | 测试部署能创建并进入运行或可诊断状态。 | 未达到时回到对应页面核对权限、筛选条件和配置状态 |
...
- 快速开始是接入顺序引导，不替代云账号、资源池、授权和部署资产页面的字段确认。
- 首次接入建议用测试租户和测试部署验证链路，再开放给真实用户。
- 每一步失败都应回到对应功能页处理，不要在快速开始页反复刷新判断结果。
