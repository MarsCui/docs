...
| --- | --- | --- | --- | --- |
| 标签名称 | 必填 | 文本 | `长上下文` | 用户可见的标签名称。 |
| 标签分组 | 必填 | 下拉选择 | `模型能力` | 用于组织标签体系。 |
| 适用对象 | 必填 | 多选 | `模型` | 标签可绑定的对象类型。 |
| 排序 | 否 | 数字 | `20` | 控制展示顺序。 |
| 状态 | 必填 | 枚举 | `启用` | 控制标签是否可被使用。 |
...
| 检查项 | 成功表现 | 异常时处理 |
| --- | --- | --- |
| 标签能在模型编辑或应用编辑页被绑 | 标签能在模型编辑或应用编辑页被绑定。 | 未达到时回到对应页面核对权限、筛选条件和配置状态 |
| 用户侧发现页能按标签筛选出正确结 | 用户侧发现页能按标签筛选出正确结果。 | 未达到时回到对应页面核对权限、筛选条件和配置状态 |
| 停用标签后不再作为新增绑定项出现 | 停用标签后不再作为新增绑定项出现。 | 未达到时回到对应页面核对权限、筛选条件和配置状态 |
## 常见问题
