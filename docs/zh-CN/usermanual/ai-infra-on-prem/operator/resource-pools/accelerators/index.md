...
| 参数 | 是否必填 | 说明 | 配置建议 |
| --- | --- | --- | --- |
| 厂商 | 必填 | 加速卡厂商。 | 与真实硬件厂商保持一致。 |
| 架构 | 必填 | 加速卡架构。 | 按页面下拉或真实硬件架构选择。 |
| 系列 | 必填 | 加速卡所属系列。 | 与型号、厂商保持一致。 |
| 型号 | 必填 | 加速卡型号。 | 与集群节点实际上报设备型号保持一致。 |
| 单卡显存 GiB | 必填 | 单卡显存容量。 | 按页面单位填写，避免 GiB/GB 混用。 |
| 计算能力 | 可选 | CUDA 或硬件能力版本。 | 没有明确值时不要臆造。 |
| 精度模式 | 条件必填 | 算力配置对应的精度模式。 | 与页面支持项和硬件能力一致。 |
| 峰值算力（TFLOPS） | 条件必填 | 对应精度模式下的峰值算力。 | 只填写已确认的公开或硬件上报数据。 |
| 加速卡规格指标 | 条件必填 | 绑定已有规格指标或新建加速卡规格指标。 | 核对 k8s-key、selector-key 与节点实际上报标签。 |
| 适配状态 | 必填 | 是否已适配平台资源识别和调度。 | 未适配时不要对用户侧开放。 |
| 操作 | 系统生成 | 新建、编辑、导入/导出、保存等入口。 | `保存` 会提交真实配置；学习或截图时不要误触。 |
