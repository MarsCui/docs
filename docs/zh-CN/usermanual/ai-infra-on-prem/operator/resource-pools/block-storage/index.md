...
| 参数 | 是否必填 | 说明 | 配置建议 |
| --- | --- | --- | --- |
| 块存储类型 | 必填 | 块存储后端类型。 | 按页面实际支持类型选择。 |
| 集群名称 | 必填 | 块存储所属或连接的集群名称。 | 与实际存储集群保持一致。 |
| Mon 节点 | 必填 | Ceph Monitor 节点地址列表。 | 文档中只使用占位符，不写真实地址。 |
| FSID | 必填 | Ceph 集群唯一标识。 | 与目标存储集群一致。 |
| 存储池 | 必填 | 承载块卷的存储池。 | 确认容量、配额和权限。 |
| 管理员 Key | 必填 | 管理员 Key 内容或凭据材料。 | 只在系统表单中填写，不写入文档、截图或工单。 |
| 超配比例 | 必填 | 逻辑容量和物理容量的超分比例。 | 按容量规划谨慎填写。 |
| 租户配额限制 | 必填 | 租户可用容量限制。 | 与租户容量策略一致。 |
| 物理阈值 | 必填 | 物理容量告警或限制阈值。 | 不要超过实际容量安全边界。 |
| 逻辑阈值 | 必填 | 逻辑容量告警或限制阈值。 | 与超配比例联动核对。 |
| 每卷快照限制 | 必填 | 单个卷可保留的快照数量。 | 结合容量和备份策略配置。 |
| 描述 | 否 | 组件用途、边界或维护说明。 | 只记录非敏感说明。 |
| 操作 | 系统生成 | 注册组件、测试连接、提交、搜索、重置等入口。 | `提交` 会提交真实配置；学习或截图时不要误触。 |
