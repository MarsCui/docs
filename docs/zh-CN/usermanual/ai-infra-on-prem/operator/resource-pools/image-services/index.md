...
2. 点击 `注册`、`新增` 或页面真实注册入口。
3. 按页面字段填写组件名称、镜像仓库地址、Endpoint、认证方式、访问凭据和证书配置。
4. 根据页面要求选择关联地域、绑定集群、项目同步范围或同步策略。
...
| 参数 | 是否必填 | 说明 | 配置建议 |
| --- | --- | --- | --- |
| 组件名称 | 必填 | 镜像组件展示名称。 | 使用能体现仓库用途、地域或环境的名称。 |
| 仓库地址 | 必填 | 镜像仓库访问地址。 | 文档只使用占位说明，不写真实仓库地址。 |
| Endpoint | 必填 | 平台和集群访问镜像仓库的服务地址。 | 确认平台侧、目标集群节点和容器运行时均可访问。 |
| 认证方式 | 条件必填 | 镜像拉取、推送或同步使用的认证方式。 | 按仓库能力选择 Robot 账号、访问账号或页面支持的方式。 |
| 访问凭据 | 条件必填 | 认证所需账号、密码、Token 或密钥材料。 | 只在系统表单中填写，不写入文档、截图或工单。 |
| 证书策略 | 条件必填 | 私有证书、证书链或 TLS 校验策略。 | 私有仓库需确认集群侧信任链。 |
| 关联地域 | 条件必填 | 镜像组件可用的地域范围。 | 与资源池、用户入口和镜像项目可见范围保持一致。 |
| 绑定集群 | 条件必填 | 可访问该镜像组件的集群。 | 绑定前确认集群网络、DNS 和容器运行时配置。 |
| 项目同步范围 | 否 | 平台同步项目、命名空间或镜像列表的范围。 | 使用最小必要范围，避免暴露无关项目。 |
| 同步状态 | 系统生成 | 镜像组件同步或探测状态。 | 注册后关注同步状态、更新时间和错误提示。 |
| 操作 | 否 | 支持注册、编辑、同步、测试连接、绑定或删除等操作。 | 高风险操作前确认地域、集群和用户侧影响。 |
...
| 注册入口可见 | 页面显示 `注册`、`新增` 或真实注册入口。 | 检查运营方权限、License 和页面配置。 |
| 注册表单可打开 | 点击入口后可查看组件名称、仓库地址、Endpoint、认证方式和证书配置字段。 | 检查路由、权限和前端错误。 |
| 必填字段校验正常 | 未填写组件名称、仓库地址、认证信息或地域范围时出现校验提示。 | 按页面提示补齐字段，不绕过校验。 |
