...
| Navigation path | Personal > Projects |
| Page route | /user/personal/projects |
...
The expected result is not visible on the `Projects` page, or the available action does not match the current business expectation.
...
- The action may be restricted because it affects projects records and related status.

**Handling:**

1. Reset filters and search again from `Personal > Projects`.
