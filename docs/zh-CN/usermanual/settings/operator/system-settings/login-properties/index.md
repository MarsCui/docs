...
| 适用角色 | 运营方管理员 |
| 导航路径 | 系统设置 > 登录配置 |
| 页面路由 | /operator/system-settings/login-properties |
...
1. 进入 `系统设置 > 登录配置`。
2. 选择 `登录安全`、`注册验证码`、`账号找回` 或 `Google 登录`。
3. 查看当前策略说明和配置值。
4. 点击 `编辑` 查看配置编辑入口。
5. 修改前确认是否会影响登录、验证码发送或账号找回。

## 参数说明

| 字段名称 | 是否必填 | 字段类型 | 示例 | 说明 |
| --- | --- | --- | --- | --- |
| 验证码开关 | 否 | 布尔 | 启用 | 控制验证码是否生效。 |
| 验证码有效期 | 否 | 时间 | 5 分钟 | 控制验证码可用时长。 |
| 账号找回 | 否 | 布尔 | 启用 | 控制是否允许用户找回账号。 |
| Google 登录 | 否 | 布尔 | 启用 | 控制第三方登录入口。 |
| 保存 | 否 | 按钮 | 保存 | 提交登录配置变更。 |
