...
| 组织 | 条件必填 | 文本 | `示例组织 A` | 生成或核对结算单时用于限定对象。 |
| 账户类型 | 条件必填 | 枚举 | `平台清分账户` | 核对流水时用于区分清分、收益或其他账户。 |
| 异常类型 | 条件必填 | 枚举 | `未配对划转` | 巡检中心排查时用于选择下一步。 |
| 结算单号 | 条件必填 | 文本 | `SETTLE-202607-ORG001` | 跟踪结算单详情、状态和入账确认。 |
...
2. 查看平台清分账户、平台收益账户等账户的余额、收入、支出和更新时间。
...
账期、组织、交易类型或账户类型选择不一致，也可能存在入账确认中、退款、调账或清分延迟。
...
- 截图、工单、评论和交付记录中不要暴露真实组织、客户、金额、流水号、账户编号或内部处理意见。
