...
AGIOne 在 AI Infra On-Prem 和 AI Infra On-Cloud 中都提供模型部署资产管理能力。On-Prem 通过模型配置、显存测算、框架配置和推理模板沉淀本地部署参数；On-Cloud 通过推理镜像、推理框架和模型库组织云上部署资产。两类资产是否可用，都取决于运营方配置、目标模型、算力规格、镜像和推理引擎验证结果。
...
| 部署范围 | 组成 | 用户手册入口 |
|---|---|---|
| **On-Prem** | 模型配置、显存测算、框架配置、推理模板 | [模型配置](../../usermanual/ai-infra-on-prem/operator/templates/model-config/) / [显存测算](../../usermanual/ai-infra-on-prem/operator/templates/vram-config/) / [框架配置](../../usermanual/ai-infra-on-prem/operator/templates/frameworks/) / [推理模板](../../usermanual/ai-infra-on-prem/operator/templates/inference-templates/) |
...
- **On-Cloud 部署资产**：由运营方按云平台和地域维护推理镜像、推理框架和模型库；
