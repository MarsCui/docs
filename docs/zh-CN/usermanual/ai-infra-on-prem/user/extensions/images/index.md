...
| 检查项 | 成功表现 | 异常时处理 |
| --- | --- | --- |
| 项目出现在 我的镜像（My Im | 项目出现在 `我的镜像（My Images）` 列表中。 | 未达到时回到对应页面核对权限、筛选条件和配置状态 |
| 项目下可查看镜像数量、配额使用和 | 项目下可查看镜像数量、配额使用和推送入口。 | 未达到时回到对应页面核对权限、筛选条件和配置状态 |
...
| 检查项 | 成功表现 | 异常时处理 |
| --- | --- | --- |
| docker push 命令执行 | `docker push` 命令执行成功。 | 未达到时回到对应页面核对权限、筛选条件和配置状态 |
| 返回镜像服务页面 | 返回镜像服务页面，点击 `同步（Sync）`。 | 未达到时回到对应页面核对权限、筛选条件和配置状态 |
| 在项目下看到新镜像和标签 | 在项目下看到新镜像和标签。 | 未达到时回到对应页面核对权限、筛选条件和配置状态 |
| 在创建在线 IDE、运行实例或模 | 在创建在线 IDE、运行实例或模型服务时可以选择该镜像。 | 未达到时回到对应页面核对权限、筛选条件和配置状态 |
