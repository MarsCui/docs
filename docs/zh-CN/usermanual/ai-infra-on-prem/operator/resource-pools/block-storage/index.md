...
块存储组件像给实例挂载的独立硬盘供应入口，负责把 Ceph RBD 或兼容块设备能力接入平台。用户创建需要持久化卷的实例时，平台会按这里的存储类、容量和访问模式去申请并挂载块卷。
...
| --- | --- | --- | --- | --- |
| 组件名称 | 必填 | 文本 | `ceph-rbd-prod` | 块存储组件展示名称。 |
| 访问协议 | 必填 | 枚举 | `RBD` | 块存储访问协议。 |
| Endpoint | 必填 | URL | `https://storage.example.com` | 组件访问入口，文档中使用占位符。 |
...
- 同一块卷不要随意跨节点多写挂载，除非底层存储和应用明确支持该模式。
...
| 目标地域可以绑定块存储能力 | 目标地域可以绑定块存储能力。 | 未达到时回到对应页面核对权限、筛选条件和配置状态 |
| 测试工作负载能创建、挂载、卸载并 | 测试工作负载能创建、挂载、卸载并释放块卷。 | 未达到时回到对应页面核对权限、筛选条件和配置状态 |
| 容量统计与底层存储系统保持一致 | 容量统计与底层存储系统保持一致。 | 未达到时回到对应页面核对权限、筛选条件和配置状态 |
