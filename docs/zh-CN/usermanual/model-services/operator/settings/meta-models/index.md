...
| --- | --- | --- | --- | --- |
| 元模型名称 | 必填 | 文本 | `Qwen Text` | 模型能力抽象名称。 |
| 输入输出模态 | 必填 | 多选 | `文本 -> 文本` | 声明模型支持的数据类型。 |
| 协议 | 必填 | 多选 | `openai/chat_completions` | 模型兼容调用协议。 |
| Token 限制 | 必填 | 数字 | `128000` | 上下文、输入或输出 Token 上限。 |
...
| 检查项 | 成功表现 | 异常时处理 |
| --- | --- | --- |
| 新增的元模型在列表中可见 | 新增的元模型在列表中可见。 | 未达到时回到对应页面核对权限、筛选条件和配置状态 |
| 发布模型或配置模板时能选择到该元 | 发布模型或配置模板时能选择到该元模型。 | 未达到时回到对应页面核对权限、筛选条件和配置状态 |
| 协议、模态、Token 限制与模 | 协议、模态、Token 限制与模型实际能力一致。 | 未达到时回到对应页面核对权限、筛选条件和配置状态 |
| 默认参数在体验或调用测试中能按预 | 默认参数在体验或调用测试中能按预期生效。 | 未达到时回到对应页面核对权限、筛选条件和配置状态 |
## 常见问题
