...
| --- | --- | --- | --- | --- |
| 来源名称 | 必填 | 文本 | `dashscope-cn` | 模型来源展示名称。 |
| 区域 | 必填 | 文本 | `cn-shanghai` | 来源服务所在区域。 |
| Endpoint | 必填 | URL | `https://api.example.com/v1` | 上游基础地址，示例使用占位符。 |
...
| 检查项 | 成功表现 | 异常时处理 |
| --- | --- | --- |
| 模型来源在列表中显示连通或可用状 | 模型来源在列表中显示连通或可用状态。 | 未达到时回到对应页面核对权限、筛选条件和配置状态 |
| 模板和模型发布流程能选择该来源 | 模板和模型发布流程能选择该来源。 | 未达到时回到对应页面核对权限、筛选条件和配置状态 |
| 请求头、区域和 Base URL | 请求头、区域和 Base URL 与上游服务要求一致。 | 未达到时回到对应页面核对权限、筛选条件和配置状态 |
| 连通性测试失败时能看到明确错误提 | 连通性测试失败时能看到明确错误提示。 | 未达到时回到对应页面核对权限、筛选条件和配置状态 |
## 常见问题
