...
| --- | --- | --- | --- | --- |
| 模型名称 | 必填 | 文本 | `qwen2-72b` | 平台展示和模板引用的模型名称。 |
| 模型路径 | 必填 | 路径 / URI | `s3://models/qwen2-72b` | 框架加载模型文件的位置。 |
| 参数来源 | 必填 | 枚举 | `模板默认` | 说明参数来自模型配置、模板默认值还是用户输入。 |
...
| 检查项 | 成功表现 | 异常时处理 |
| --- | --- | --- |
| 模型出现在列表中 | 模型出现在列表中。 | 未达到时回到对应页面核对权限、筛选条件和配置状态 |
| 模型版本状态符合预期 | 模型版本状态符合预期。 | 未达到时回到对应页面核对权限、筛选条件和配置状态 |
| 创建推理模板时可以选择该模型 | 创建推理模板时可以选择该模型。 | 未达到时回到对应页面核对权限、筛选条件和配置状态 |
| 用户侧部署模板中能看到与可见范围 | 用户侧部署模板中能看到与可见范围匹配的模型。 | 未达到时回到对应页面核对权限、筛选条件和配置状态 |
