...
| `operator` | 平台运营方 | 资源准备、治理配置、配额、监控和审核 | 代替模型提供方发布模型，或作为普通用户消费模型 |
| `provider` | 模型提供方 | 发布和维护模型、聚合模型、审核提交、客户调用和收益 | 平台级身份治理，或审核自己发布的内容 |
| `enduser` | 普通用户与模型使用方 | 部署可用服务、发现和体验模型、API 调用和个人用量 | 发布或审核模型，管理平台级资源 |
...
| 查看个人调用和用量 | 否 | 获得授权时查看运营范围 | 查看自身范围 | 查看自身范围 |
