...
`模型收益` 用于查看模型收入、收益明细、结算周期、币种和客户贡献，帮助模型提供方核对收益来源和结算状态。
...
| --- | --- | --- | --- | --- |
| 时间范围 | 必填 | 日期范围 | `2026-07` | 收益统计周期。 |
| 模型 | 否 | 下拉选择 | `qwen-plus` | 按模型筛选收益。 |
| 客户 | 否 | 下拉选择 | `customer-a` | 按客户查看收益贡献。 |
| 收益金额 | 系统生成 | 数字 | `120 Credits` | 按币种设置折算后的收益。 |
...
| 检查项 | 成功表现 | 异常时处理 |
| --- | --- | --- |
| 收益概览与收益明细汇总一致 | 收益概览与收益明细汇总一致。 | 未达到时回到对应页面核对权限、筛选条件和配置状态 |
| 切换模型、客户或时间范围后 | 切换模型、客户或时间范围后，趋势图和明细同步变化。 | 未达到时回到对应页面核对权限、筛选条件和配置状态 |
| 结算状态、币种单位和展示精度符合 | 结算状态、币种单位和展示精度符合币种设置。 | 未达到时回到对应页面核对权限、筛选条件和配置状态 |
## 常见问题
...
客户调用不在当前统计周期内，收益规则未覆盖该模型，或客户调用仍处于结算延迟状态。

**处理方式：**

先核对统计时间、模型和客户筛选；再进入客户调用日志确认是否有成功调用；如有调用但无收益，联系运营方核对收益规则和结算任务。
