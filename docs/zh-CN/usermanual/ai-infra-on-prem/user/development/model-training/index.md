...
下图展示创建运行实例页面，用于填写镜像、规格、启动命令和存储挂载。
...
| --- | --- | --- | --- | --- |
| 任务名称 | 必填 | 文本 | `train-job-001` | 训练任务展示名称。 |
| 镜像 | 必填 | 下拉选择 | `pytorch:2.3` | 训练任务镜像。 |
| 启动命令 | 必填 | 文本 | `python train.py` | 训练入口命令。 |
| 资源规格 | 必填 | 下拉选择 | `1GPU-16C-64G` | 训练任务使用规格。 |
...
| 检查项 | 成功表现 | 异常时处理 |
| --- | --- | --- |
| 实例出现在列表中 | 实例出现在列表中。 | 未达到时回到对应页面核对权限、筛选条件和配置状态 |
| 状态进入运行中、成功或符合任务类 | 状态进入运行中、成功或符合任务类型的状态。 | 未达到时回到对应页面核对权限、筛选条件和配置状态 |
| 日志中没有镜像拉取、命令执行或挂 | 日志中没有镜像拉取、命令执行或挂载错误。 | 未达到时回到对应页面核对权限、筛选条件和配置状态 |
| 输出目录中产生预期文件 | 输出目录中产生预期文件。 | 未达到时回到对应页面核对权限、筛选条件和配置状态 |
