...
| --- | --- | --- | --- | --- |
| 模型名称 | 必填 | 文本 | `DeepSeek-V4-Pro` | 模型展示名称。 |
| Model ID | 必填 | 文本 | `provider/model/version` | 调用模型时使用的标识。 |
| 供应方 | 必填 | 下拉选择 | `AGIOneSystem` | 提供模型实例的渠道。 |
| 推荐标签 | 否 | 标签 | `推荐` | 平台推荐或状态标签。 |
| Full URL | 必填 | URL | `https://api.example.com/v1/chat/completions` | 完整调用地址示例。 |
...
| 检查项 | 成功表现 | 异常时处理 |
| --- | --- | --- |
| 模型详情页能展示能力、供应方、价 | 模型详情页能展示能力、供应方、价格、上下文和快速开始信息。 | 未达到时回到对应页面核对权限、筛选条件和配置状态 |
| 筛选条件变化后 | 筛选条件变化后，模型列表结果同步变化。 | 未达到时回到对应页面核对权限、筛选条件和配置状态 |
| 复制的调用示例使用脱敏占位凭据 | 复制的调用示例使用脱敏占位凭据。 | 未达到时回到对应页面核对权限、筛选条件和配置状态 |
## 常见问题
