...
| --- | --- |
| 适用角色 | 运营方管理员 |
| 导航路径 | API 流控 > API 流控概览 |
| 页面路由 | /operator/api-rate-control/overview |
| 管理对象 | 全局请求量、拦截请求、统计超限、启用规则和节点状态 |
| 典型用途 | 查看 API 流控概览、请求趋势和 Top 规则排行 |
...
![API 流控概览](./images/rate-control-overview.png)
...
1. 进入 `API 流控 > API 流控概览`。
2. 选择开始时间和结束时间。
3. 点击 `搜索` 查看统计结果。
4. 查看请求量、拦截请求和统计超限指标。
5. 查看 Top5 规则排行，定位触发频繁的规则。
6. 如需进一步排查，进入观测审计或规则管理页面。

## 参数说明

| 字段名称 | 是否必填 | 字段类型 | 示例 | 说明 |
| --- | --- | --- | --- | --- |
| 开始时间 | 是 | 日期时间 | 2026-07-13 09:00 | 统计 API 请求、拦截和超限数据的起始时间。 |
| 结束时间 | 是 | 日期时间 | 2026-07-13 10:00 | 统计 API 请求、拦截和超限数据的结束时间。 |
| 请求量 | 系统生成 | 数字 | 12,000 | 当前时间范围内的请求总量。 |
| 拦截请求 | 系统生成 | 数字 | 320 | 被流控规则拦截的请求数量。 |
| 启用规则 | 系统生成 | 数字 | 18 | 当前处于启用状态的规则数量。 |
| Top5 规则排行 | 系统生成 | 排行列表 | 规则 A | 命中次数靠前的流控规则，用于快速定位异常来源。 |
| 详情 | 否 | 操作按钮 | 详情 | 查看规则排行或统计明细的入口。 |
