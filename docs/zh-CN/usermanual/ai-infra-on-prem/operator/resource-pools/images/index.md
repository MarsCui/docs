...
1. 进入 `AI Infra > On-Prem > 资源池管理 > 镜像管理`，确认镜像组件已接入且镜像列表可正常加载。
...
| 参数 | 是否必填 | 说明 | 配置建议 |
| --- | --- | --- | --- |
| 客户端工具 | 条件必填 | 用于构建、登录和推送镜像的本地工具。 | 使用 Docker、Podman 或页面支持的实际客户端工具。 |
| 镜像仓库 | 必填 | 镜像所在 registry。 | 文档示例只使用 `<registry>` 占位符，不写真实仓库地址。 |
| 项目/命名空间 | 必填 | 镜像仓库中的项目、组织或 namespace。 | 使用 `<project>` 占位符示例，实际配置按仓库权限填写。 |
| 镜像名称 | 必填 | 镜像展示名称或仓库中的 image 名称。 | 名称应体现框架、用途或运行环境。 |
| 镜像标签 | 必填 | 镜像版本标签。 | 避免在生产场景只使用 `latest`。 |
| 镜像地址 | 必填 | 完整镜像地址，例如 `<registry>/<project>/<image>:<tag>`。 | 提交前核对 registry、project、image 和 tag。 |
| 镜像类型 | 否 | 镜像用途类型，如开发、训练或推理。 | 与后续作业、IDE 或推理模板使用场景一致。 |
| 架构 | 否 | 镜像 CPU 架构或硬件架构。 | 与目标集群节点架构匹配。 |
| 同步状态 | 系统生成 | 平台同步镜像后的状态。 | 推送后回到平台刷新或同步检查。 |
| 操作 | 否 | 支持镜像上传、同步、编辑标签、查看或删除等操作。 | 高风险操作前确认影响范围。 |
