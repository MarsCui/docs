...
| --- | --- |
| 适用角色 | 运营方 |
| 导航路径 | AI Infra > On-Prem > 资源池管理 > 资源规格 |
| 页面路由 | /powerone/resourcepool/flavor/list |
| 管理对象 | 规格名称、CPU、内存、加速卡、加速卡数量、规格指标、关联集群和启用状态 |
| 典型用途 | 定义作业资源套餐、限制用户资源申请范围、关联集群后开放给作业或模型服务使用 |
...
2. 点击 `新增` 或页面真实新增入口。
3. 填写规格名称，建议体现 CPU、内存、加速卡型号、卡数和适用场景。
...
| 参数 | 是否必填 | 说明 | 配置建议 |
| --- | --- | --- | --- |
| 规格名称 | 必填 | 用户创建在线 IDE、运行实例、训练任务或模型服务时选择的规格名称。 | 名称应体现 CPU、内存、加速卡型号、卡数和适用场景。 |
...
| 必填字段校验正常 | 未填写规格名称、规格指标或数量时出现校验提示。 | 按页面提示补齐字段，不绕过校验。 |
