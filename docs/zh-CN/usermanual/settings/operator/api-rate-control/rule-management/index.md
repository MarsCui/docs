...
| 适用角色 | 运营方管理员 |
| 导航路径 | API 流控 > 规则管理 |
| 页面路由 | /operator/api-rate-control/rule-management |
| 管理对象 | API 流控规则、API Pattern、规则状态和发布状态 |
| 典型用途 | 查询、创建、编辑、启停和发布 API 流控规则 |
...
1. 进入 `API 流控 > 规则管理`。
2. 输入规则名称或 API Pattern 后点击 `搜索`。
3. 查看规则的计数范围、模式、额度和窗口。
4. 点击 `编辑` 或 `复制` 查看规则配置入口。
5. 对 `新建规则`、`发布全部规则版本`、启用状态变更等操作，先确认影响范围。

## 参数说明

| 字段名称 | 是否必填 | 字段类型 | 示例 | 说明 |
| --- | --- | --- | --- | --- |
| 规则名称 | 必填 | 文本 | 示例规则 A | 用于识别规则。 |
| API 路径 | 必填 | 文本 | /v1/models | 规则匹配的接口路径。 |
| 阈值 | 必填 | 数值 | 100 次/分钟 | 触发统计或拦截的限制。 |
| 模式 | 必填 | 枚举 | 拦截 | 统计或拦截模式。 |
| 状态 | 否 | 枚举 | 启用 | 判断规则是否生效。 |
