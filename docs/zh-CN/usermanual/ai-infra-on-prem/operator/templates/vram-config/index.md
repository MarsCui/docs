...
`显存测算配置` 用于维护模型部署时的显存估算规则，帮助推理模板根据模型、精度、KV Token、并发、上下文长度和动态表达式推荐资源规格。

| 项目 | 内容 |
| --- | --- |
| 适用角色 | 运营方 |
| 导航路径 | 模板 > 显存测算 |
| 页面路由 | `/powerone/fast-build-v2/vram` |
| 管理对象 | 显存公式、精度、KV Token、因子表单、动态表达式和推荐规格 |
| 典型用途 | 降低用户选择规格错误导致的部署失败 |

### 新手理解

显存配置像部署前的容量估算器，用模型规模、精度、上下文长度和 KV Token 估算需要多少显存，避免服务启动后才发现放不下。
...
![显存测算列表](./images/vram-config-list-browser.png)
...
![显存测算配置](./images/vram-config-create-precision-browser.png)

### 参数说明

| 字段名称 | 是否必填 | 字段类型 | 示例 | 说明 |
| --- | --- | --- | --- | --- |
| 模型规模 | 是 | 文本 / 数字 | `72B` | 模型参数规模，用于估算权重显存。 |
| 精度 | 是 | 枚举 | `BF16` | 影响权重、激活和 KV Cache 的显存占用。 |
| KV Token | 是 | 数字 | `32768` | 用于估算上下文和并发下的 KV Cache 占用。 |
| 上下文长度 | 是 | 数字 | `8192` | 模型服务允许的最大输入输出上下文。 |
...
1. 规则出现在列表中。
2. 推理模板可以引用该显存规则。
3. 使用不同模型、精度、KV Token 和并发组合时，推荐规格符合预期。
4. 用户选择过小规格时，页面能给出限制或提示。
