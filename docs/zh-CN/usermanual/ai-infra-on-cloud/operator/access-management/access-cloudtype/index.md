...
::: warning 安全提示
云平台接入涉及云厂商类型、API 能力、地域映射和资源发现规则。文档、截图、工单和评论中不要暴露真实账号 ID、内部 Endpoint、AK/SK 或授权策略；新增云平台前应确认最小权限和可审计范围。
:::

## 功能概述

`接入云平台` 用于维护云平台类型、私有云地址、启用状态和接入说明，支撑多云调度、资源授权和模型部署流程。
...
| --- | --- | --- | --- | --- |
| 云平台名称 | 是 | 文本 | `AGIOne Private Cloud` | 面向运营方展示的云平台名称，避免内部代号。 |
| 云平台类型 | 是 | 枚举 | `私有云` | 区分公有云、私有云或专属云。 |
| 访问地址 | 条件必填 | URL | `https://cloud.example.com` | 私有云入口示例，文档中只写占位符。 |
| 启用状态 | 是 | 枚举 | `启用` | 控制后续接入账号和资源池是否可选择。 |
