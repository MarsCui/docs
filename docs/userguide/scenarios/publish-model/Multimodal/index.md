...
The model accepts every declared input modality, passes protocol testing, and publishes with accurate capability labels.
...
- Prepare the model source, identifier, credential, endpoint, and harmless text-plus-media samples.
...
2. Select the multimodal meta-model and model source, then enter the upstream request URL, credential, and exact source model ID.

![Model source and meta-model information](./images/step-1-model-source_meta-model-information.png)

3. Confirm that the model type is `Multimodal` so the platform exposes the correct capability and configuration fields.
...
| Check | Pass Criteria |
| --- | --- |
| 1 | Every advertised modality passes a controlled protocol test. |
| 2 | Unsupported modality combinations are not exposed. |
| 3 | Billing and limits cover multimodal input size and output behavior. |
| 4 | The published model works in the corresponding Playground. |
...
| Capability labels are misleading | Declared modalities, tested combinations, model version, and marketplace description |
