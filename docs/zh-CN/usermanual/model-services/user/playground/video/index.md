...
`视频生成` 用于选择视频模型、输入素材、配置抽帧和生成参数，并查看视频生成或处理结果。
...
| 检查项 | 成功表现 | 异常时处理 |
| --- | --- | --- |
| 页面返回视频摘要、问答或生成结果 | 页面返回视频摘要、问答或生成结果。 | 未达到时回到对应页面核对权限、筛选条件和配置状态 |
| 抽帧、时长和输出格式参数变化后 | 抽帧、时长和输出格式参数变化后，结果符合预期。 | 未达到时回到对应页面核对权限、筛选条件和配置状态 |
| 失败时能看到请求 ID、错误码或 | 失败时能看到请求 ID、错误码或参数限制提示。 | 未达到时回到对应页面核对权限、筛选条件和配置状态 |
## 常见问题
