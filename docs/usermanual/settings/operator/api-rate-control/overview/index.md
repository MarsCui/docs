...
| Navigation path | API Rate Control > API Rate Control Overview |
| Page route | /operator/api-rate-control/overview |
...
![API Rate Control Overview](./images/rate-control-overview.png)
...
1. Go to `API Rate Control > API Rate Control Overview`.
...
4. Click the visible `View API Rate Control Overview` entry when it is available.
...
The expected result is not visible on the `Overview` page, or the available action does not match the current business expectation.
...
- The action may be restricted because it affects overview records and related status.

**Handling:**

1. Reset filters and search again from `API Rate Control > API Rate Control Overview`.
