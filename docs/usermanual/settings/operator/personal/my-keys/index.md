...
| Page route | /operator/personal/my-keys |
...
![My Keys](./images/my-keys-list.png)
...
The expected result is not visible on the `My Keys` page, or the available action does not match the current business expectation.
...
- The action may be restricted because it affects my keys records and related status.

**Handling:**

1. Reset filters and search again from `Personal > My Keys`.
