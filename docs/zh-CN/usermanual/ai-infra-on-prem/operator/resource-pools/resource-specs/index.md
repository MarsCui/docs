...
| --- | --- | --- | --- | --- |
| 规格名称 | 必填 | 文本 | `gpu-a100-1-16c-64g` | 用户创建实例时选择的规格。 |
| CPU | 必填 | 数字 | `16` | 规格包含的 CPU 核数。 |
| 内存 | 必填 | 文本 | `64GiB` | 规格包含的内存。 |
...
| 检查项 | 成功表现 | 异常时处理 |
| --- | --- | --- |
| 规格出现在列表中 | 规格出现在列表中。 | 未达到时回到对应页面核对权限、筛选条件和配置状态 |
| 目标集群详情中可关联该规格 | 目标集群详情中可关联该规格。 | 未达到时回到对应页面核对权限、筛选条件和配置状态 |
| 用户创建作业时能选择该规格 | 用户创建作业时能选择该规格。 | 未达到时回到对应页面核对权限、筛选条件和配置状态 |
