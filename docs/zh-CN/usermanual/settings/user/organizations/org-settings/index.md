...
组织设置页用于维护服务商组织的全局默认规则，包括 Credit 余额预警、项目默认值、成员额度默认值和额度申请设置。

| 项目 | 内容 |
| --- | --- |
| 适用角色 | 服务商管理员 |
| 导航路径 | 组织&设置 > 组织设置 |
| 页面路由 | /user/organizations/org-settings |
| 管理对象 | Credit 余额预警、项目默认值、成员额度策略和组织级默认规则 |
| 典型用途 | 维护组织默认规则和额度预警配置 |

### 新手理解

组织设置页像组织默认规则面板，用来设置新项目、新成员或额度申请的默认策略。改动后通常影响后续创建对象，已有对象仍需在各自页面单独核对。

### 术语速查

| 术语 | 含义 | 处理建议 |
| --- | --- | --- |
| 组织设置 | 组织级默认配置入口。 | 改动前确认适用范围。 |
...
- 组织设置变更通常影响后续对象，不要默认已有项目和成员会同步变化。
...
先确认当前组织；再检查组织设置权限；如配置确实为空，由组织管理员按默认规则补齐后保存。
### 为什么组织设置保存按钮不可用？

**问题现象：**

组织设置项可见，但保存、重置或修改默认规则的按钮不可点击。
