...
| --- | --- |
| 适用角色 | 运营方 |
| 导航路径 | AI Infra > On-Prem > 资源池管理 > 地域/可用区 |
...
1. 进入 `AI Infra > On-Prem > 资源池管理 > 地域可用区`。
...
7. 点击最终 `确定` 前，再次核对所属地域、可用区 ID、显示名称和描述。
...
| 所属地域 | 必填 | 下拉选择 | `武汉` | 可用区所属的上级地域，提交前必须核对。 |
...
| 真实提交后记录正确 | 新地域或可用区出现在列表中，且地域-可用区层级关系正确。 | 重新核对地域ID、可用区ID和所属地域。 |
