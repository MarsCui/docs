...
| 参数 | 是否必填 | 说明 | 配置建议 |
| --- | --- | --- | --- |
| AI 加速卡类别 | 条件必填 | 新增 AI 加速卡指标时选择的类别。 | 与加速卡管理中的硬件分类保持一致。 |
| AI 加速卡指标 | 条件必填 | 新增 AI 加速卡规格指标时选择的指标类型。 | 用于区分加速卡类指标。 |
| 其他指标 | 条件必填 | CPU、内存或页面支持的非加速卡指标。 | 与资源规格展示和调度口径一致。 |
| 指标名称 | 必填 | 规格指标在页面中的展示名称。 | 命名应能表达资源类型和单位。 |
| 单位 | 必填 | 指标展示或计量单位。 | 与容量统计和资源规格页面保持一致。 |
| k8s-key | 条件必填 | Kubernetes 调度资源 key。 | 必须与 Kubernetes 节点实际上报资源 key 一致。 |
| selector-key | 条件必填 | 加速卡型号、节点标签或设备筛选 key。 | 必须与加速卡型号或节点标签一致。 |
| 操作 | 系统生成 | 新增、编辑、导入/导出、删除等入口。 | `确定` 会提交真实配置；学习或截图时不要误触。 |
